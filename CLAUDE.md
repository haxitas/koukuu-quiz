...
- 選択画面トップに「過去に間違えた問題集（誤答N問）」ボタンを1つだけ表示(0件なら非表示)。クリックすると `data/index.json` 記載の全期を fetch して結合し、誤答バンクから最大10問(`REVIEW_MAX`、期の新しい順→ファイル内順)を出題する。全期間へ展開しても再実装不要(自動的に含まれる)。
