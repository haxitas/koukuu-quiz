...
- **Phase1(R8-02は完了、法規は令和期全展開済み)**: 実物PDF→JSONの半自動パイプラインを確立(`scripts/` 参照)。R8-02は無線工学14・法規20・英語12・英会話7=全53問(4科目すべて実データ)。加えて**法規は令和期13期(R2-02〜R8-02)全て投入済み**(計260問、全問公式正答と完全一致確認済み)。並列エージェントに複数期を割り当てる際は「1期終わるごとに都度保存」を必ず指示すること(MUSENN-PHASE_NOTES.md参照)。次は琥珀の優先順位どおり英語・英会話。工学と平成期以前は指示があるまで対象外(リサーチ沼防止のため自走で広げない)。
