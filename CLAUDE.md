...
- `presented` = その回に出題した問題idの一覧(通常回=科目の全問、復習回=出題した誤答のみ)。**誤答バンク(復習モードの対象)は可変ストアを持たず、`attempts` から都度導出する**(`quiz-core.currentMistakes`)。判定規則: あるidの直近の出題(presentedに含まれた回)時に誤答だったら「現在の誤答」。正解すればその回以降は含まれず自然に消え、誤答すれば残り続ける。attempts自体は書き換えないので append-only の原則(憲法1)と両立する。
- 出題画面では、通常回はその時点の誤答バンクに入っている問題に「前回まちがえた」バッジ、復習回は誤答バンクから最大10問(`REVIEW_MAX`、出題順)を選んで出題する。
